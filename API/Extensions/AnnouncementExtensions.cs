...
public static class AnnouncementExtensions
{
    ...
    public static IQueryable<Announcement> IndustryFilter(this IQueryable<Announcement> query,
        string industry)
    {
        if (string.IsNullOrEmpty(industry)) return query;

        var lowerCaseIndustryFilterTerm = industry.Trim().ToLower();

        return query.Where(c => c.Industry.ToLower().Contains(lowerCaseIndustryFilterTerm));
    }
}